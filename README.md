...
这是一个基于 Next.js、Prisma 和 SQLite 的课程空间系统，包含课程、专题、收件箱、小组、笔记、通讯录、云盘、课程 Copilot、直播和 AI 文档建课等功能。
...
- 教师先在 Copilot 设置中绑定自己的云盘文件夹；文件夹及子目录会作为课程资料实时开放给学生只读访问。
- 课程资料上传和 AI 文档导入会写入该文件夹。学生可以在私密对话中主动 `@` 文档或静态图片，不会默认加载整个文件夹。
- 教师可上传最大 10MB 的 Markdown/ZIP Skill，测试后启用；学生也可以不选 Skill，直接进行普通对话。
- 每个对话最多添加 5 个文件，文档正文合计不超过 100,000 字符，单张图片不超过 10MB、图片合计不超过 20MB。
- 学生每日调用上限由 `COPILOT_DAILY_LIMIT` 配置，默认 100 次；教师只查看近 7 日匿名汇总，不查看学生对话内容。
