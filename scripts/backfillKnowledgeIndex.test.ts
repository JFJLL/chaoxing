/**
 * One-time maintenance task: backfills the local FTS5 knowledge index for
 ...
 *
 * Runs under vitest (not tsx) because mupdf is ESM with top-level await and
 * cannot be bundled by tsx/esbuild. Usage: npm run backfill:knowledge
 */
import { PrismaClient } from "@prisma/client";
import { describe, it } from "vitest";
import { extractText } from "@/lib/document/extractText";
import { hasKnowledgeDocument, indexKnowledgeDocument } from "@/lib/document/knowledgeDb";
...
describe("knowledge index backfill", () => {
  it("indexes legacy drive files and imports into the local FTS5 knowledge database", async () => {
