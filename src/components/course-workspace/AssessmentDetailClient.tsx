  ...
  return (
    <div className="space-y-5">
      ...
      {canManage && status === "DRAFT" ? (
        <section className="space-y-6 rounded-2xl border border-[#F9ECE7] bg-[#FDF3F0]/40 p-4 sm:p-5">
        ...
        <section className="space-y-4">
          ...
          {records.map((target) => <form key={target.id} action={(formData) => grade(target, formData)} className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50 p-5"><div className="flex items-center justify-between"><div><p className="font-medium">{target.userName}</p><p className="text-xs text-slate-500">{target.status} · {target.submittedAt ? new Date(target.submittedAt).toLocaleString("zh-CN") : "尚未提交"}</p></div><Badge tone={target.status === "GRADED" ? "green" : "orange"}>{target.score ?? 0} 分</Badge></div>{target.answers.map((answer) => { const question = questions.find((item) => item.id === answer.questionId); return <div key={answer.id} className="rounded-xl bg-white p-4"><p className="text-sm font-medium">{question?.stem}</p><p className="mt-2 whitespace-pre-wrap text-sm text-slate-600">学生回答：{answer.response || "未作答"}</p><p className="mt-1 text-xs text-[#8E3425]">标准答案：{question?.answer}</p><div className="mt-3 grid gap-2 md:grid-cols-[120px_1fr]"><Input name={`score-${answer.id}`} type="number" min={0} max={answer.maxPoints} step="0.5" defaultValue={answer.score ?? 0} /><Input name={`feedback-${answer.id}`} defaultValue={answer.feedback ?? ""} placeholder="单题评语" /></div></div>; })}<Textarea name="feedback" defaultValue={target.feedback ?? ""} placeholder="整体评语" /><div className="flex gap-2"><Button type="submit" disabled={busy}><CheckCircle2 className="h-4 w-4" />保存评分</Button>{kind === "assignment" && target.status !== "RETURNED" ? <Button type="button" variant="secondary" disabled={busy} onClick={() => returnSubmission(target)}>退回重做</Button> : null}</div></form>)}
