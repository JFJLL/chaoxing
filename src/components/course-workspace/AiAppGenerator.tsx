...
function statusTone(status: AiArtifactStatus) {
  ...
  if (status === "QUEUED" || status === "GENERATING") return "bg-[#FDF3F0] text-[#8E3425]";
...
export function AiAppGenerator({
  ...
  return (
    // On xl screens the workspace is capped to the viewport so both columns end
    // on the same line; each column scrolls internally instead of stretching the
    // page. Below xl the columns stack and follow the normal document flow.
    <div className="grid gap-5 xl:h-[calc(100vh-120px)] xl:min-h-[560px] xl:grid-cols-[360px_minmax(0,1fr)] xl:items-stretch">
      <aside className="flex flex-col rounded-2xl border border-slate-100 bg-white p-5 shadow-sm xl:min-h-0 xl:overflow-y-auto">
        ...
        <form onSubmit={submit} className={app.appType === "html_courseware" ? "hidden" : "space-y-4"}>
                          ...
                          <div className="flex items-start justify-between gap-2">
                            ...
                            {document.sections.length > 0 ? (
                              <button
                                ...
                                className="mt-0.5 flex shrink-0 items-center gap-1 text-xs font-medium text-[#A8402F] hover:text-[#8E3425]"
                              >
                                {isExpanded ? "收起" : "展开"}
          ...
          {!(["lesson_plan", "courseware", "ppt_courseware", "question_generation"] as CourseAiAppType[]).includes(app.appType) ? <div className="space-y-3">
            ...
            <label className="space-y-1 text-sm font-medium text-slate-700"><span>难度</span><select value={options.difficulty} onChange={(event) => updateOption("difficulty", event.target.value)} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]"><option>基础</option><option>提高</option><option>综合</option><option>挑战</option></select></label>
          </div> : null}

          {app.appType === "question_generation" ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><label className="space-y-1 text-sm font-medium text-slate-700"><span>题型</span><select value={options.questionType} onChange={(event) => updateOption("questionType", event.target.value)} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]"><option>混合</option><option>单选</option><option>多选</option><option>简答</option></select></label><label className="space-y-1 text-sm font-medium text-slate-700"><span>题量</span><input type="number" min={3} max={12} value={options.questionCount} onChange={(event) => updateOption("questionCount", Number(event.target.value))} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]" /></label></div> : null}
          {app.appType === "lesson_plan" ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><label className="space-y-1 text-sm font-medium text-slate-700"><span>课时分钟</span><input type="number" min={30} max={180} value={options.lessonMinutes} onChange={(event) => updateOption("lessonMinutes", Number(event.target.value))} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]" /></label><label className="space-y-1 text-sm font-medium text-slate-700"><span>教法</span><select value={options.teachingMethod} onChange={(event) => updateOption("teachingMethod", event.target.value)} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]"><option>讲授结合实践</option><option>任务驱动</option><option>案例研讨</option><option>翻转课堂</option></select></label></div> : null}
          {app.appType === "courseware" ? <div className="space-y-3">{sourceArtifactId ? <div className="rounded-xl border border-[#F9ECE7] bg-[#FDF3F0]/70 p-3 text-sm">{recommendationLoading ? <p className="flex items-center gap-2 text-[#6F281D]"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" />AI正在分析教案并推荐课件页数……</p> : recommendationError ? <div className="space-y-2 text-amber-800"><p>AI页数建议暂不可用，你仍可以手动填写页数。</p><button type="button" onClick={retrySlideRecommendation} className="font-medium text-[#8E3425] underline underline-offset-4">重新分析</button></div> : recommendedSlideCount !== null ? <div className="space-y-2 text-[#522017]"><p className="font-medium">AI分析建议：{recommendedSlideCount}页</p>{recommendationReason ? <p className="text-xs leading-5 text-[#6F281D]">原因：{recommendationReason}</p> : null}<button type="button" onClick={adoptRecommendedSlideCount} className="inline-flex items-center rounded-lg bg-[var(--cx-blue)] px-3 py-1.5 text-xs font-medium text-white">采用{recommendedSlideCount}页</button></div> : <button type="button" onClick={requestSlideRecommendation} className="inline-flex items-center gap-1.5 font-medium text-[#8E3425] underline underline-offset-4"><Sparkles className="h-4 w-4" aria-hidden="true" />让 AI 分析教案并推荐课件页数</button>}</div> : null}<div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1"><label className="space-y-1 text-sm font-medium text-slate-700"><span>课件页数</span><input type="number" min={SLIDE_COUNT_MIN} max={SLIDE_COUNT_MAX} value={options.slideCount} onChange={(event) => { slideCountTouchedRef.current = true; setSlideCountTouched(true); updateOption("slideCount", Number(event.target.value)); }} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]" /></label><label className="space-y-1 text-sm font-medium text-slate-700"><span>风格</span><select value={options.coursewareStyle} onChange={(event) => updateOption("coursewareStyle", event.target.value)} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]"><option>课堂讲授</option><option>案例分析</option><option>实训操作</option><option>复习总结</option></select></label></div></div> : null}
          {app.appType === "paper_assembly" ? <label className="space-y-1 text-sm font-medium text-slate-700"><span>试卷总分</span><input type="number" min={30} max={150} value={options.paperScore} onChange={(event) => updateOption("paperScore", Number(event.target.value))} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]" /></label> : null}
          ...
          {app.appType === "courseware" || app.appType === "ppt_courseware" ? <div className="space-y-3"><div className="rounded-xl bg-[#FDF3F0] p-3 text-sm leading-6 text-[#6F281D]"><p className="font-medium">{app.appType === "courseware" ? "只从已确认教案生成 AI课件" : "将已确认 AI课件逐页生成整页视觉 PPT"}</p><p className="mt-1">{app.appType === "ppt_courseware" ? "每页生成成功后消耗 1 积分；供应商或系统失败时，未生成页面的积分会自动退回。" : "系统固定记录来源版本，上游修改不会静默覆盖当前产物。"}</p></div><CollapsibleSourcePanel title={app.appType === "courseware" ? "来源教案" : "来源AI课件"} panelId="ai-source-panel" summary={<>当前：{selectedCoursewareSource ? `${selectedCoursewareSource.title} · v${selectedCoursewareSource.version}` : "尚未选择"}<span className="mt-0.5 block">点击展开选择来源{app.appType === "courseware" ? "教案" : "AI课件"}</span></>}><label className="space-y-1 text-sm font-medium text-slate-700"><span>{app.appType === "courseware" ? "来源教案" : "来源AI课件"}</span><select value={sourceArtifactId} onChange={(event) => setSourceArtifactId(event.target.value)} className="h-10 w-full rounded-xl border border-slate-200 px-3 text-sm font-normal outline-none focus:border-[#D07865]"><option value="">请选择已确认{app.appType === "courseware" ? "教案" : "AI课件"}</option>{coursewareSources.map((source) => <option key={source.id} value={source.id}>{source.title} · v{source.version}</option>)}</select></label></CollapsibleSourcePanel>{app.appType === "ppt_courseware" && sourceArtifactId ? <div className={`rounded-xl p-3 text-sm ${insufficientPptCredits ? "bg-amber-50 text-amber-900" : "bg-slate-50 text-slate-700"}`}><p className="font-medium">本次生成预算：{pptPageCount ?? "正在读取"} 页 × 1 积分</p><p className="mt-1">当前可用：{creditBalance ?? "正在读取"} 积分</p>{insufficientPptCredits ? <Link href="/space/billing" className="mt-2 inline-block font-medium text-[#8E3425] underline underline-offset-4">积分不足，去充值</Link> : null}</div> : null}{missingApprovedCourseware ? <p className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">还没有可用的已确认{app.appType === "courseware" ? "教案" : "AI课件"}。</p> : null}</div> : null}

          {app.appType !== "ppt_courseware" ? <><div><label htmlFor="ai-app-prompt" className="text-sm font-medium text-slate-700">生成要求</label><textarea id="ai-app-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} placeholder={`补充${app.title}要求`} className="mt-2 min-h-24 w-full resize-y rounded-xl border border-slate-200 p-3 text-sm outline-none focus:border-[#D07865]" /></div><div className="rounded-xl bg-[#FDF3F0] p-3 text-xs leading-5 text-[#8E3425]">{structuredPrompt()}</div></> : null}
        ...
        <div className="mt-6 flex flex-col"><h2 className="shrink-0 text-sm font-semibold text-slate-900">历史产物</h2><div className="mt-3 max-h-[420px] space-y-2 overflow-y-auto pr-1 xl:max-h-none xl:overflow-visible">
          {artifacts.map((artifact) => {
            ...
            return (
              <article key={artifact.id} className={`rounded-xl border p-3 text-sm transition ${selected?.id === artifact.id ? "border-[#F0C8BE] bg-[#FDF3F0]" : "border-slate-100 bg-slate-50"}`}>
                {renamingId === artifact.id ? (
                  <div className="flex items-center gap-1">
                    <input
                      ...
                      maxLength={200}
                      aria-label="产物名称"
                      className="min-w-0 flex-1 rounded-lg border border-slate-300 px-2 py-1 text-sm outline-none focus:border-[#D07865]"
                    />
                  ...
                  <div className="flex items-start gap-1">
                    <button type="button" onClick={() => selectArtifact(artifact.id)} className="min-w-0 flex-1 text-left">
                      <span className={`line-clamp-1 font-medium ${selected?.id === artifact.id ? "text-[#8E3425]" : "text-slate-700"}`}>{artifact.title}</span>
                ...
                <div className="mt-3 flex items-center gap-2 border-t border-slate-200/70 pt-2">
                  {choices.length === 1 ? (
                    <button type="button" disabled={!exportable || exportingId === artifact.id} onClick={() => exportArtifact(artifact, choices[0]!)} className="inline-flex items-center gap-1 text-xs font-medium text-[#A8402F] disabled:text-slate-300">
                      <Download className="h-3.5 w-3.5" aria-hidden="true" />{exportingId === artifact.id ? "导出中" : "导出"}
                    </button>
                  ) : choices.length > 1 ? (
                    <details className="relative">
                      <summary className={`inline-flex cursor-pointer list-none items-center gap-1 text-xs font-medium ${exportable ? "text-[#A8402F]" : "pointer-events-none text-slate-300"}`}>
                        <Download className="h-3.5 w-3.5" aria-hidden="true" />{exportingId === artifact.id ? "导出中" : "导出"}
                      </summary>
