            ...
            >
              ...
              <div className="mt-auto flex flex-wrap gap-2 pt-4" aria-label={`${workflow.title}包含的步骤`}>
                {workflow.includes.map((item) => (
                  <span key={item} className="rounded-full border border-slate-200/80 bg-white px-2.5 py-1 text-xs text-slate-500 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">{item}</span>
                ))}
              </div>
            </Link>
          );
