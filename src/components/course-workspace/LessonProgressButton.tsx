...
export function LessonProgressButton({ courseId, lessonId, completed }: { courseId: string; lessonId: string; completed: boolean }) {
  ...
  return <div className="flex items-center gap-2"><Button type="button" variant="secondary" className="h-8" disabled={busy} onClick={toggle}>{completed ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : <Circle className="h-4 w-4" />}{completed ? "已自报完成" : "我已学完"}</Button>{error ? <span className="text-xs text-red-600">{error}</span> : null}</div>;
}
