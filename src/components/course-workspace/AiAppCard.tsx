...
export function AiAppCard({ app, courseId }: { app: CourseAiAppDefinition; courseId: string }) {
  const body = (
    <div
      className={clsx(
        "group h-full rounded-xl border border-slate-100 bg-white p-5 text-left shadow-sm transition",
        app.enabled ? "hover:-translate-y-0.5 hover:border-blue-100 hover:shadow-md" : "opacity-75"
      )}
    >
      <div className="flex items-start gap-4">
        <span className={clsx("flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br text-white", colorClasses[app.color])}>
