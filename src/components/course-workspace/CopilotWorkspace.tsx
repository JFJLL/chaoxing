    ...
    <div className="space-y-5">
      {canManage ? (
        <div className="inline-flex rounded-xl bg-slate-100 p-1">
          <button type="button" onClick={() => setView("chat")} className={`rounded-lg px-4 py-2 text-sm font-medium ${view === "chat" ? "bg-white text-[#8E3425] shadow-sm" : "text-slate-600"}`}><Sparkles className="mr-2 inline h-4 w-4" />测试 AI智能体</button>
          <button type="button" onClick={() => setView("settings")} className={`rounded-lg px-4 py-2 text-sm font-medium ${view === "settings" ? "bg-white text-[#8E3425] shadow-sm" : "text-slate-600"}`}><Settings2 className="mr-2 inline h-4 w-4" />AI智能体设置</button>
        </div>
      ) : null}
            ...
            <div className="mt-4 min-h-0 flex-1 space-y-2 overflow-y-auto pr-1">
              {conversations.map((conversation) => (
                <div key={conversation.id} className={`group flex items-center rounded-xl ${selectedId === conversation.id ? "bg-white shadow-sm" : "hover:bg-white/70"}`}>
                  ...
                  <button type="button" aria-label="重命名对话" onClick={() => void renameConversation(conversation)} className="p-1 text-slate-400 hover:text-[#A8402F]"><Pencil className="h-3.5 w-3.5" /></button>
            ...
            <div className="shrink-0 border-b border-slate-100 p-4">
              <div className="flex flex-wrap items-center gap-3">
                <label className="relative">
                  ...
                  <select value={selected?.activeSkill?.id ?? ""} onChange={(event) => void chooseSkillFromToolbar(event.target.value)} disabled={streaming || busy === "context" || busy === "conversation"} className="h-10 appearance-none rounded-xl border border-slate-200 bg-white pl-9 pr-9 text-sm text-slate-700 outline-none focus:border-[#D07865]">
                  ...
                  <Sparkles className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-[#A8402F]" />
              ...
              {selected?.attachments.length ? (
                <div className="mt-3 flex flex-wrap gap-2">
                  {selected.attachments.map((attachment) => (
                    <span key={attachment.id ?? attachment.name} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs ${attachment.available ? "bg-[#FDF3F0] text-[#8E3425]" : "bg-red-50 text-red-700"}`}>
              ...
              <div className="space-y-6">
                {selected?.messages.map((message) => message.role === "USER" ? (
                  <div key={message.id} className="mx-auto flex w-full max-w-3xl justify-end">
                    <article className="max-w-[85%] rounded-2xl rounded-br-md bg-[#A8402F] px-4 py-3 text-white shadow-sm sm:max-w-[78%]">
                      {message.skillName || message.contextFiles.length ? <p className="mb-2 text-xs text-[#F9ECE7]">{message.skillName ? `Skill：${message.skillName}` : ""}{message.skillName && message.contextFiles.length ? " · " : ""}{message.contextFiles.length ? "已使用课程资料" : ""}</p> : null}
                ...
                {pendingUserMessage && pendingUserMessage.conversationId === selected?.id ? (
                  <div className="mx-auto flex w-full max-w-3xl justify-end">
                    <article className="max-w-[85%] rounded-2xl rounded-br-md bg-[#A8402F] px-4 py-3 text-white shadow-sm sm:max-w-[78%]">
                      {pendingUserMessage.skillName || pendingUserMessage.contextFiles.length ? <p className="mb-2 text-xs text-[#F9ECE7]">{pendingUserMessage.skillName ? `Skill：${pendingUserMessage.skillName}` : ""}{pendingUserMessage.skillName && pendingUserMessage.contextFiles.length ? " · " : ""}{pendingUserMessage.contextFiles.length ? "已使用课程资料" : ""}</p> : null}
                ...
                {!selected?.messages.length && !pendingUserMessage && !streaming ? <div className="mx-auto flex min-h-64 max-w-md flex-col items-center justify-center text-center"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#FDF3F0]"><Bot className="h-6 w-6 text-[#A8402F]" /></span><h2 className="mt-4 font-semibold text-slate-900">开始使用 AI智能体</h2><p className="mt-2 text-sm leading-6 text-slate-500">直接提问即可开始；需要特定方法时选择 Skill，需要结合课程材料时添加文件。</p></div> : null}
              </div>
            </div>

            <div className="shrink-0 border-t border-slate-100 bg-white px-4 py-3 sm:px-6 sm:py-4">
              <div className="mx-auto w-full max-w-3xl rounded-2xl border border-slate-200 bg-white p-3 shadow-sm transition focus-within:border-[#E5A597] focus-within:ring-2 focus-within:ring-[#FDF3F0]">
                <textarea
                  ref={composerRef}
                  value={draft}
                  rows={1}
            ...
            {settingsLoaded && canBindRootState ? (
              <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
                <select
                  ...
                  className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-700 outline-none focus:border-[#D07865]"
                >
