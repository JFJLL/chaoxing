...
export function QuestionBankClient({ courseId, initialQuestions }: { courseId: string; initialQuestions: QuestionBankItem[] }) {
  ...
  return <>
    <div className="space-y-3">
      {questions.map((question) => <button key={question.id} type="button" onClick={() => open(question)} className="group w-full rounded-2xl border border-slate-100 bg-slate-50 p-5 text-left transition hover:border-blue-200 hover:bg-blue-50/40 focus:outline-none focus:ring-2 focus:ring-blue-200">
        <div className="flex items-start justify-between gap-4"><div><ScrollText className="h-6 w-6 text-emerald-600" /><p className="mt-3 text-xs font-medium text-slate-400">{typeLabel(question.type)} · v{question.version}</p><h2 className="mt-1 font-semibold text-slate-900">{question.stem}</h2><p className="mt-2 text-sm text-blue-700">答案：{question.answer}</p><p className="mt-1 text-xs text-slate-400">{question.sourceTitle} · 来源产物 v{question.sourceVersion ?? "-"} · {question.id}</p></div><span className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 group-hover:border-blue-200 group-hover:text-blue-700"><Eye className="h-4 w-4" />查看与编辑</span></div>
      </button>)}
