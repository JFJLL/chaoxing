...
export function DriveClient({ files, courses }: { files: Array<{ id: string; name: string; kind: string; size: number }>; courses: Array<{ id: string; title: string }> }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-3">{files.map((file) => <article key={file.id} className="flex flex-wrap items-center gap-3 rounded-md border border-[var(--cx-border)] p-3"><span className="font-medium">{file.kind === "folder" ? "文件夹" : "文件"}：{file.name}</span><span className="text-sm text-slate-500">{file.size} bytes</span>{file.kind === "file" ? <a href={`/api/drive/${file.id}?download=1`} className="text-[var(--cx-blue)]">下载</a> : null}<Button type="button" variant="secondary" className="h-8" onClick={() => share(file.id)}>分享</Button>{courses[0] ? <Button type="button" variant="secondary" className="h-8" onClick={() => attach(file.id, courses[0].id)}>添加到课程资料</Button> : null}</article>)}</div>
    </div>
  );
}
