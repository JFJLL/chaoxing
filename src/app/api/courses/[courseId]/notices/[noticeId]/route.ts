...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  const existing = await db.announcement.findFirst({ where: { id: noticeId, courseId } });
  if (!existing) return NextResponse.json({ error: "通知不存在" }, { status: 404 });
  const notice = await db.announcement.update({
    where: { id: noticeId },
    data: {
      ...parsed.data,
      publishAt: parsed.data.publishAt === undefined ? undefined : parsed.data.publishAt ? new Date(parsed.data.publishAt) : null
    }
  });
