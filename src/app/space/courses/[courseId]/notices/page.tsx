...
export default async function NoticesPage({ params }: PageProps) {
  ...
  const notices = await db.announcement.findMany({
    ...
    include: { author: { select: { name: true } }, reads: { where: { userId: user.id }, select: { readAt: true } }, _count: { select: { reads: true } } },
  ...
  return (
    <FanyaCourseShell user={user} course={course} activeTab="notices">
      <CourseModulePanel title="通知" description={canManage ? "发布课程公告并查看学生已读情况。" : "查看课程公告和教学提醒。"}>
        <NoticesClient courseId={courseId} canManage={canManage} notices={notices.map((notice) => ({ id: notice.id, title: notice.title, body: notice.body, status: notice.status, publishAt: notice.publishAt?.toISOString() ?? null, pinned: notice.pinned, authorName: notice.author.name, readAt: notice.reads[0]?.readAt.toISOString() ?? null, readCount: notice._count.reads }))} />
      </CourseModulePanel>
    </FanyaCourseShell>
  );
}
