...
export default async function AnalyticsPage({ params }: PageProps) {
  ...
  const [lessons, lessonProgress, attendanceSessions, attendanceRecords, assignments, exams] = await Promise.all([
    db.lesson.findMany({ where: { chapter: { courseId } }, select: { id: true, estimatedMinutes: true } }),
    db.lessonProgress.findMany({ where: { userId: { in: studentIds }, completedAt: { not: null }, lesson: { chapter: { courseId } } }, select: { userId: true, lessonId: true } }),
  ...
  const indicatorsFor = (studentId: string) => {
    ...
    const completedLessonIds = new Set(lessonProgress.filter((row) => row.userId === studentId).map((row) => row.lessonId));
    return buildLearningIndicators({ lessons: { completed: completedLessonIds.size, total: lessons.length, rate: weightedLessonCompletionRate(lessons, completedLessonIds) }, attendance: { present: countFor(attendanceRecords, studentId), total: attendanceSessions }, assignments: { submitted: assignments.reduce((sum, item) => sum + (item.submissions.some((submission) => submission.userId === studentId) ? 1 : 0), 0), total: assignments.length }, exams: { gradedScore: gradedAttempts.reduce((sum, item) => sum + (item.score ?? 0), 0), gradedMaxScore: gradedAttempts.reduce((sum, item) => sum + item.max, 0) } });
  };
