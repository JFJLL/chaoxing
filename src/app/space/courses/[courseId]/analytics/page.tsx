...
import { countCompletedNoticeEngagements, noticeEngagementCompletionRate } from "@/lib/teaching/lessonCompletion";
...
export default async function AnalyticsPage({ params }: PageProps) {
  ...
  const [announcements, announcementReads, driveFileDownloads, attendanceSessions, attendanceRecords, assignments, exams] = await Promise.all([
    db.announcement.findMany({
      where: { courseId, status: "PUBLISHED", publishAt: { lte: now } },
      select: {
        id: true,
        attachments: {
          select: {
    ...
    db.announcementRead.findMany({ where: { userId: { in: studentIds }, announcement: { courseId } }, select: { announcementId: true, userId: true } }),
    db.driveFileDownload.findMany({ where: { userId: { in: studentIds } }, select: { driveFileId: true, userId: true } }),
  ...
  const notices = announcements.map((announcement) => ({
    id: announcement.id,
    attachments: announcement.attachments.map((attachment) => ({
      driveFileId: attachment.driveFileId,
      deleted: attachment.driveFile.deletedAt !== null
    }))
  }));
  ...
  const indicatorsFor = (studentId: string) => {
    ...
    const completedNotices = countCompletedNoticeEngagements(notices, announcementReads, driveFileDownloads, studentId);
    return buildLearningIndicators({ lessons: { completed: completedNotices, total: notices.length, rate: noticeEngagementCompletionRate(notices, announcementReads, driveFileDownloads, studentId) }, attendance: { present: countFor(attendanceRecords, studentId), total: attendanceSessions }, assignments: { submitted: assignments.reduce((sum, item) => sum + (item.submissions.some((submission) => submission.userId === studentId) ? 1 : 0), 0), total: assignments.length }, exams: { gradedScore: gradedAttempts.reduce((sum, item) => sum + (item.score ?? 0), 0), gradedMaxScore: gradedAttempts.reduce((sum, item) => sum + item.max, 0) } });
  };
  ...
  return <FanyaCourseShell user={user} course={course} activeTab="analytics"><CourseModulePanel title={canManage ? "学情分析" : "我的学习"} description={canManage ? "课时完成按学生已读通知与下载通知附件计算；签到、作业和考试保持独立口径。" : "课时进度按你已读课程通知与下载通知附件计算，不使用不透明综合分。"}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{cards.map((card) => { const Icon = card.icon; return <article key={card.label} className="rounded-2xl border border-slate-100 bg-slate-50 p-5"><Icon className="h-6 w-6 text-[#A8402F]" /><p className="mt-4 text-sm text-slate-500">{card.label}</p><p className="mt-1 text-2xl font-semibold text-slate-950">{card.value}</p></article>; })}</div>{canManage ? <section className="mt-5 rounded-2xl border border-slate-100 p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold text-slate-900">学生学习明细</h2><p className="mt-1 text-xs text-slate-500">课时完成为已读通知且已下载附件（如有）的通知占比。</p></div><Badge tone="blue">{students.length} 人</Badge></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="py-3">学生</th><th>课时完成</th><th>签到率</th><th>作业完成</th><th>考试平均</th></tr></thead><tbody>{students.map((student) => <tr key={student.id} className="border-b border-slate-100"><td className="py-3"><p className="font-medium text-slate-900">{student.name}</p><p className="text-xs text-slate-400">{student.email}</p></td><td>{rateLabel(student.indicators.lessonCompletionRate)}</td><td>{rateLabel(student.indicators.attendanceRate)}</td><td>{rateLabel(student.indicators.assignmentCompletionRate)}</td><td>{rateLabel(student.indicators.examAverageRate)}</td></tr>)}</tbody></table></div></section> : null}</CourseModulePanel></FanyaCourseShell>;
}
