import { BarChart3, BookOpenCheck, Bot, ClipboardCheck, Radio, UsersRound } from "lucide-react";
...
export default async function AnalyticsPage({ params }: PageProps) {
  ...
  const [lessonTotal, lessonProgress, attendanceSessions, attendanceRecords, assignments, exams, coachTaskCount, coachCompleted] = await Promise.all([
    ...
    db.aiCoachTask.count({ where: { courseId, status: "PUBLISHED" } }),
    db.courseAiConversation.findMany({ where: { courseId, userId: { in: studentIds }, kind: "COACH", status: "COMPLETED" }, select: { userId: true, coachTaskId: true } })
  ]);
  ...
  const indicatorsFor = (studentId: string) => {
    ...
    return buildLearningIndicators({ lessons: { completed: countFor(lessonProgress, studentId), total: lessonTotal }, attendance: { present: countFor(attendanceRecords, studentId), total: attendanceSessions }, assignments: { submitted: assignments.reduce((sum, item) => sum + (item.submissions.some((submission) => submission.userId === studentId) ? 1 : 0), 0), total: assignments.length }, exams: { gradedScore: gradedAttempts.reduce((sum, item) => sum + (item.score ?? 0), 0), gradedMaxScore: gradedAttempts.reduce((sum, item) => sum + item.max, 0) }, aiCoach: { completed: new Set(coachCompleted.filter((item) => item.userId === studentId).map((item) => item.coachTaskId).filter(Boolean)).size, total: coachTaskCount } });
  };
  ...
  const mine = students[0]?.indicators ?? buildLearningIndicators({ lessons: { completed: 0, total: 0 }, attendance: { present: 0, total: 0 }, assignments: { submitted: 0, total: 0 }, exams: { gradedScore: 0, gradedMaxScore: 0 }, aiCoach: { completed: 0, total: 0 } });
  const cards = canManage ? [{ label: "选课学生", value: course.enrollments.length, icon: UsersRound }, { label: "已结束签到", value: attendanceSessions, icon: Radio }, { label: "已发布作业", value: assignments.length, icon: ClipboardCheck }, { label: "已发布考试", value: exams.length, icon: BarChart3 }] : [{ label: "课时完成", value: rateLabel(mine.lessonCompletionRate), icon: BookOpenCheck }, { label: "签到率", value: rateLabel(mine.attendanceRate), icon: Radio }, { label: "作业完成", value: rateLabel(mine.assignmentCompletionRate), icon: ClipboardCheck }, { label: "考试平均", value: rateLabel(mine.examAverageRate), icon: BarChart3 }, { label: "AI陪练", value: rateLabel(mine.aiCoachCompletionRate), icon: Bot }];
  return <FanyaCourseShell user={user} course={course} activeTab="analytics"><CourseModulePanel title={canManage ? "学情分析" : "我的学习"} description={canManage ? "所有指标均来自真实课时、签到、作业、考试和 AI 陪练记录。" : "仅展示你自己的真实学习记录，不使用不透明综合分。"}><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">{cards.map((card) => { const Icon = card.icon; return <article key={card.label} className="rounded-2xl border border-slate-100 bg-slate-50 p-5"><Icon className="h-6 w-6 text-blue-600" /><p className="mt-4 text-sm text-slate-500">{card.label}</p><p className="mt-1 text-2xl font-semibold text-slate-950">{card.value}</p></article>; })}</div>{canManage ? <section className="mt-5 rounded-2xl border border-slate-100 p-5"><div className="flex items-center justify-between"><h2 className="font-semibold text-slate-900">学生学习明细</h2><Badge tone="blue">{students.length} 人</Badge></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead><tr className="border-b text-slate-500"><th className="py-3">学生</th><th>课时完成</th><th>签到率</th><th>作业完成</th><th>考试平均</th><th>AI陪练</th></tr></thead><tbody>{students.map((student) => <tr key={student.id} className="border-b border-slate-100"><td className="py-3"><p className="font-medium text-slate-900">{student.name}</p><p className="text-xs text-slate-400">{student.email}</p></td><td>{rateLabel(student.indicators.lessonCompletionRate)}</td><td>{rateLabel(student.indicators.attendanceRate)}</td><td>{rateLabel(student.indicators.assignmentCompletionRate)}</td><td>{rateLabel(student.indicators.examAverageRate)}</td><td>{rateLabel(student.indicators.aiCoachCompletionRate)}</td></tr>)}</tbody></table></div></section> : null}</CourseModulePanel></FanyaCourseShell>;
}
